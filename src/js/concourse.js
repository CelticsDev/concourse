...
if (window.location.href.indexOf("nba.com") > -1) {
  ...
  var feeds = {
    todaysScores:
      "http://data.nba.com/data/v2015/json/mobile_teams/nba/2017/scores/00_todays_scores.json",
    celticsRoster:
      "http://data.nba.com/data/v2015/json/mobile_teams/nba/2017/teams/celtics_roster.json",
    awayRoster: function(awayTn) {
      return (
        "http://data.nba.com/data/v2015/json/mobile_teams/nba/2017/teams/" +
        awayTn +
        "_roster.json"
      );
    },
    bioData:
      "http://io.cnn.net/nba/nba/.element/media/2.0/teamsites/celtics/json/bio-data.json",
    playercard: function(pid) {
      return (
        "http://data.nba.com/data/v2015/json/mobile_teams/nba/2017/players/playercard_" +
        pid +
        "_02.json"
      );
    },
    ...
    gamedetail: function(gid) {
      return (
        "http://data.nba.com/data/v2015/json/mobile_teams/nba/2017/scores/gamedetail/" +
        gid +
        "_gamedetail.json"
      );
    },
    standings:
      "http://data.nba.com/data/v2015/json/mobile_teams/nba/2017/00_standings.json",
    leagueLeaders:
  ...
  var feeds = {
    todaysScores:
      "http://localhost:8888/data/mobile-stats-feed/todays_scores.json",
    celticsRoster:
      "http://localhost:8888/data/mobile-stats-feed/celtics_roster.json",
    awayRoster: function(awayTn) {
    ...
    playercard: function(pid) {
      return (
        "http://localhost:8888/data/mobile-stats-feed/playercards/playercard-" +
        pid +
        ".json"
      );
    },
...
function generateTimeline(selectedPlayer) {
  ...
  for (var i = 0; i < seasonsPlayed; i++) {
    var teamAbbreviation =
      rosterObj.celtics.roster[selectedPlayer].stats.sa[i].ta;
    var traded =
      rosterObj.celtics.roster[selectedPlayer].stats.sa[i].spl.length;
    var segmentInner = "";
    var title = "";
    var seasonYearText =
      rosterObj.celtics.roster[selectedPlayer].stats.sa[i].val;
    if (
      i === 0 ||
      teamAbbreviation !==
        rosterObj.celtics.roster[selectedPlayer].stats.sa[i - 1].ta
    ) {
    ...
    if (traded) {
      for (var x = 0; x < traded; x++) {
        ...
        teamAbbreviation =
          rosterObj.celtics.roster[selectedPlayer].stats.sa[i].spl[x].ta;
        if (
          i === 0 ||
          (teamAbbreviation !==
            rosterObj.celtics.roster[selectedPlayer].stats.sa[i - 1].ta &&
    ...
    seasonYearHTML +=
      '<div class="segment"><p>' + seasonYearText + "</p></div>";
  }
  jQuery(".timeline-wrap").html(
    '<div class="timeline appended">' +
      timelineHTML +
      '</div><div class="season-year appended">' +
      seasonYearHTML +
      "</div>"
  );
}
...
function loadRosterData(awayTeam, awayTn) {
    ...
    jQuery.ajax({
      ...
      success: function(data) {
        if (data.pl.ca.hasOwnProperty("sa")) {
          rosterObj.celtics.roster[pid].stats =
            data.pl.ca.sa[data.pl.ca.sa.length - 1];
          rosterObj.celtics.roster[pid].stats.sa = data.pl.ca.sa;
        } else {
          rosterObj.celtics.roster[pid].stats = data.pl.ca;
        }
        rosterObj.celtics.roster[pid].stats.pts = round(
          rosterObj.celtics.roster[pid].stats.pts
        );
        rosterObj.celtics.roster[pid].stats.ast = round(
          rosterObj.celtics.roster[pid].stats.ast
        );
        rosterObj.celtics.roster[pid].stats.reb = round(
          rosterObj.celtics.roster[pid].stats.reb
        );
      },
      error: function() {}
    });
  ...
  for (var i = 0; i < awayRoster.t.pl.length; i++) {
    ...
    jQuery.ajax({
      ...
      success: function(data) {
        if (data.pl.ca.hasOwnProperty("sa")) {
          rosterObj.away.roster[pid].stats =
            data.pl.ca.sa[data.pl.ca.sa.length - 1];
          rosterObj.away.roster[pid].stats.sa = data.pl.ca.sa;
        } else {
          rosterObj.away.roster[pid].stats = data.pl.ca;
        }
        rosterObj.away.roster[pid].stats.pts = round(
          rosterObj.away.roster[pid].stats.pts
        );
        rosterObj.away.roster[pid].stats.ast = round(
          rosterObj.away.roster[pid].stats.ast
        );
        rosterObj.away.roster[pid].stats.reb = round(
          rosterObj.away.roster[pid].stats.reb
        );
      },
      error: function() {}
    });
  ...
  for (var i = 0; i < saIndex.length; i++) {
    ...
    if (i === 1) {
      rosterObj[pid].stats.sa[0][saIndex[i]] =
        playerCardYear.toString().substr(2, 2) +
        "-" +
        (playerCardYear + 1).toString().substr(2, 2);
    }
    if (i === 17) {
      rosterObj[pid].stats.sa[0][saIndex[i]] = [];
    }
...
function playerSpotlight(rosterObj) {
  ...
  setTimeout(function() {
    jQuery(".social-top .white-line.vertical:nth-child(odd)").addClass(
      "transition-1"
    );
    jQuery(".social-bottom .white-line.vertical:nth-child(even)").addClass(
      "transition-1"
    );
  }, 800);
  /* 2b - WHITE LINE VERTICAL */
  setTimeout(function() {
    jQuery(".social-top .white-line.vertical:nth-child(even)").addClass(
      "transition-1"
    );
    jQuery(".social-bottom .white-line.vertical:nth-child(odd)").addClass(
      "transition-1"
    );
  }, 1000);
  ...
  setTimeout(function() {
    ...
    for (var player in rosterObj.celtics.roster) {
      var headshot =
        ...
        rosterObj.celtics.roster[player].pid +
        ".png";
      jQuery(".player-box:nth-child(" + (forinCounter + 1) + ")").append(
        '<img class="appended headshot" src="' + headshot + '"/>'
      );
      jQuery(".player-box:nth-child(" + (forinCounter + 1) + ")").attr(
        "data-pid",
        rosterObj.celtics.roster[player].pid
      );
      jQuery(".player-box img").on("error", function() {
        jQuery(this).attr(
          "src",
          "https://i.cdn.turner.com/nba/nba/.element/media/2.0/teamsites/celtics/media/generic-player-light_600x438.png"
        );
      });
  ...
  setTimeout(function() {
    ...
    jQuery(".player-box:nth-child(" + playerSpotlightCounter + ")").addClass(
      "selected"
    );
    selectedPlayer = jQuery(
      ".player-box:nth-child(" + playerSpotlightCounter + ")"
    ).attr("data-pid");
    setTimeout(function() {
      jQuery(".player-box")
  ...
  setTimeout(function() {
    ...
    for (var i = 0; i < 3; i++) {
      if (i <= rosterObj.celtics.roster[selectedPlayer].bio.personal.length) {
        jQuery(".player-spotlight .bottom-wrap").append(
          '<div class="dyk-box appended"><p>' + playerFacts[i] + "</p></div>"
        );
      }
    }
    ...
    if (jQuery(".player-spotlight .bottom-wrap .dyk-box").length > 1) {
      setTimeout(function() {
        jQuery(
          ".player-spotlight .bottom-wrap .dyk-box:nth-of-type(2)"
        ).addClass("transition-2");
        jQuery(
          ".player-spotlight .bottom-wrap .dyk-box:nth-of-type(3)"
    ...
    if (jQuery(".player-spotlight .bottom-wrap .dyk-box").length > 2) {
      setTimeout(function() {
        ...
        jQuery(
          ".player-spotlight .bottom-wrap .dyk-box:nth-of-type(4)"
        ).addClass("transition-1");
      }, 20000);
    }
  }, 5000);
  ...
  setTimeout(function() {
    jQuery(
      ".player-spotlight .bottom-wrap, .player-spotlight .top-wrap"
    ).addClass("transition-2");
  }, 40000);
  /* 10 - DONE. REMOVE */
  setTimeout(function() {
...
function leaders(gid, gameStarted) {
      ...
      success: function(data) {
        ...
        for (var x = 0; x < teamLineScore.length; x++) {
          ...
          for (var p = 0; p < stats.pstsg.length; p++) {
            for (var stat in rosterObj[team].leaders) {
              rosterObj[team].leaders[stat].push([
                stats.pstsg[p].fn.toUpperCase(),
                stats.pstsg[p].ln.toUpperCase(),
                stats.pstsg[p][stat],
                stats.pstsg[p].pid
              ]);
            }
      ...
      for (var stat in rosterObj[team].leaders) {
        ...
        jQuery(
          ...
          '<span class="appended ' +
            rosterObj[team].ta +
            '">' +
            rosterObj[team].leaders[stat][i][2] +
            "</span> " +
            stat.toUpperCase()
        );
        // LEADER NAME
        if (
          rosterObj[team].leaders[stat][i][0].length +
            rosterObj[team].leaders[stat][i][1].length >=
          14
        ) {
          rosterObj[team].leaders[stat][i][0] =
            rosterObj[team].leaders[stat][i][0].substr(0, 1) + ".";
        }
        jQuery(
          ".leader-section:nth-of-type(" +
            (i + 2) +
          ...
          '<span class="appended">' +
            rosterObj[team].leaders[stat][i][0] +
            "</span> " +
            rosterObj[team].leaders[stat][i][1]
        );
        // LEADER HEADSHOT
        jQuery(
          ".leader-section:nth-of-type(" +
          ...
          "src",
          "https://ak-static.cms.nba.com/wp-content/uploads/headshots/nba/latest/1040x760/" +
            rosterObj[team].leaders[stat][i][3] +
            ".png"
        );
      }
    }
  ...
  setTimeout(function() {
    ...
    jQuery(".leader-subsection.bottom p:nth-of-type(1)").addClass(
      "transition-1"
    );
    jQuery(
      ".leaders .leader-section .underline, .leaders .leader-subsection.top"
    ).addClass(rosterObj.celtics.ta + "-bg");
  }, 1100);
  setTimeout(function() {
  ...
  setTimeout(function() {
    for (let i = 1; i < 6; i++) {
      setTimeout(function(numberString) {
        jQuery(".leaders .leader-section .leader-stat-wrap").addClass(
          "transition-" + i
        );
        jQuery(
          ".leaders .leader-section .underline, .leaders .leader-subsection.top"
        ).removeClass(rosterObj.celtics.ta + "-bg");
        jQuery(
          ".leaders .leader-section .underline, .leaders .leader-subsection.top"
        ).addClass(rosterObj.away.ta + "-bg");
        if (transitionCounter % 2 == 0) {
          setTimeout(function() {
            jQuery(
              ".leaders .leader-section .underline, .leaders .leader-subsection.top"
            ).removeClass(rosterObj.away.ta + "-bg");
            jQuery(
              ".leaders .leader-section .underline, .leaders .leader-subsection.top"
            ).addClass(rosterObj.celtics.ta + "-bg");
            jQuery(".leader-subsection.bottom p").removeClass("transition-1");
            jQuery(".leaders .leader-section .underline").addClass(
              "transition-" + i / 2
            );
            jQuery(
              ".leader-subsection.bottom p:nth-of-type(" + (i - i / 2 + 1) + ")"
            ).addClass("transition-1"); // lol
          }, 200);
        }
        transitionCounter++;
      }, 7000 * i);
  ...
  setTimeout(function() {
    jQuery(".leaders .leader-section, .leaders .leader-subsection").addClass(
      "transition-3"
    );
  }, 44100);
  setTimeout(function() {
  ...
  setTimeout(function() {
    ...
    for (var i = 1; i < 10; i++) {
      jQuery(
        ".leaders .transition-" + i + ", .leaders.transition-" + i
      ).removeClass("transition-" + i);
    }
  }, 45000);
}
...
function mobileApp() {
  ...
  var rotateScreens = setInterval(function() {
    jQuery(".app .bottom-wrap img").removeClass("active");
    jQuery(".app .feature-list p").removeClass("active");
    jQuery(".app .feature-list p:nth-of-type(" + counter + ")").addClass(
      "active"
    );
    jQuery(".app .bottom-wrap img:nth-of-type(" + counter + ")").addClass(
      "active"
    );
    if (counter == 5) {
      counter = 1;
        ...
        for (let x = 0; x < standingsData.sta.co[i].di.length; x++) {
          for (let t = 0; t < standingsData.sta.co[i].di[x].t.length; t++) {
            ...
            var rowHTML =
              ...
              "</div>";
            jQuery(
              conferences[i] + " > div:nth-child(" + (place + 1) + ")"
            ).html(rowHTML);
            jQuery(
              conferences[i] + " > div:nth-child(" + (place + 1) + ")"
...
function scoresInit(todaysScoresData) {
  ...
  if (liveScores.length != 0) {
    ...
    if (
      liveScores.length > 1 ||
      (liveScores.length == 1 && liveScores[0].h.ta != "BOS")
    ) {
      var statusCodes = [
        "1st Qtr",
        "2nd Qtr",
...
function leagueLeaders() {
  var leagueLeadersHTML =
    '<div class="title"><p>LEAGUE LEADERS</p><p>PTS</p><p>REB</p><p>AST</p><p>STL</p><p>BLK</p></div>';
  var statType = "";
  var dataIndex = [
    "RANK",
    "PLAYER_ID",
  ...
  jQuery.ajax({
    ...
    success: function(data) {
      ...
      for (let i = 0; i < leadersData.length; i++) {
        ...
        var rows = "";
        if (
          ["PTS", "REB", "AST", "STL", "BLK"].indexOf(
            leadersData[i].headers[8]
          ) !== -1
        ) {
          ...
          leagueLeadersHTML +=
            '<div class="league-leaders-wrap">' + rows + "</div>";
        }
      }
  ...
  var counter = 2;
  setInterval(function() {
    jQuery(".league-leaders-wrap, .league-leaders .title p").removeClass(
      "active"
    );
    jQuery(
      ".league-leaders-wrap:nth-of-type(" +
        counter +
        "), .league-leaders .title p:nth-of-type(" +
        counter +
        ")"
    ).addClass("active");
    if (counter == 6) {
